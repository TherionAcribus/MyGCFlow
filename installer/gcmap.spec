...
"""Construction de GCMap.exe (PyInstaller, mode dossier).

    .venv-build\\Scripts\\pyinstaller installer\\gcmap.spec --noconfirm

Résultat : dist\\GCMap\\GCMap.exe + dist\\GCMap\\_internal\\ (bibliothèques et
...
version_info = VSVersionInfo(
    ...
    kids=[
        StringFileInfo([StringTable("040C04B0", [
            StringStruct("CompanyName", "GCMap"),
            StringStruct("FileDescription", "GCMap"),
            StringStruct("FileVersion", VERSION),
            StringStruct("InternalName", "GCMap"),
            StringStruct("OriginalFilename", "GCMap.exe"),
            StringStruct("ProductName", "GCMap"),
...
a = Analysis(
    ...
    hooksconfig={},
    runtime_hooks=[],
    # Paquets parfois présents dans l'environnement mais inutiles à GCMap.
    excludes=["tkinter", "numpy", "moviepy", "matplotlib", "IPython", "pytest"],
    noarchive=False,
)

# Babel embarque les données de ~1 000 locales (30 Mo) ; GCMap n'est traduit
...
exe = EXE(
    ...
    name="GCMap",
    icon=str(ROOT / "installer" / "gcmap.ico"),
